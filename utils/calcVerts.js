 ...
 */
const getConeData = (height, radius, segments) => {
  const conePositions = [];
  const coneNormals = [];
  ...
  for (let i = 0; i < segments; i += 1) {
    const PiPos = calcTriangle(radius, segments, i);
    const PnextPos = calcTriangle(radius, segments, i + 1);

    conePositions.push(...apex);
    conePositions.push(...PiPos);
    conePositions.push(...PnextPos);

    coneNormals.push(...calcNormal(B, apex));
